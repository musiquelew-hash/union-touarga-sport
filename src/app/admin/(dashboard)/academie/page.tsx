...
import { academyCategories, currentSeason, enrollmentLabels, enrollmentStatuses, getAcademyCounts, listAcademyCoaches, listAcademyEnrollments, listAcademyGroups } from "@/lib/academy";
...
export default async function AcademyAdminPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  const [session, params, counts, coaches, groups, enrollments] = await Promise.all([
    requireAdmin(), searchParams, getAcademyCounts(), listAcademyCoaches(), listAcademyGroups(), listAcademyEnrollments(),
  ]);
      ...
      <div className="academy-admin-grid">
        ...
        <section className="admin-panel">
          ...
          <form action={createAcademyGroupAction} className="admin-form">
            ...
            <label><span>Saison</span><input name="season" defaultValue={currentSeason()} pattern="\d{4}/\d{4}" required /></label>
